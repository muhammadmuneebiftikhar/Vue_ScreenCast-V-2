<template>
  <v-container>
    ...
    <div v-for="video in videos" :key="video._id" class="flex-table">
          ...
          <div class="actions">
              <router-link :to="{ name: 'video-watch', params: {_id: video._id }}">Show</router-link>
              <router-link :to="{ name: 'admin-video-edit', params: {_id: video._id}}">Edit</router-link>
    ...
    methods: {
        deleteVideo(video){
            ...
            if(response){
                ...
                this.$store.dispatch("setSnackbar", { text: `You have successfully deleted a video ${video.name}.` });
            }
        }
    }
}
